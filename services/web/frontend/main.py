...
if page == "Login":
    ...
    username = st.text_input("Telegram username (without @)", value=st.session_state.tg_username)
...
# -------------------------
    ...
    current_level = "B1"
    try:
        r = requests.get(f"{API_BASE}/api/settings", headers=api_headers(), timeout=10)
        if r.ok:
...
# -------------------------
    ...
    current_level = "B1"
    try:
        r = requests.get(f"{API_BASE}/api/settings", headers=api_headers(), timeout=15)
        if r.ok:
            current_level = r.json().get("level", "B1")
    except requests.RequestException:
        pass
...
# -------------------------
    ...
    if not ach:
        ...
        for a in ach:
            st.write(f"✅ **{a.get('title','')}** (`{a.get('code','')}`) — {a.get('earned_at','')}")
