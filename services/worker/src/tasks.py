...
from src.utils import safe_parse_language_feedback, fallback_language_feedback

_settings = get_settings()
_model: Optional[AIWorkerModel] = None
...
def task_reply(*, level, history, message):
    ...
    try:
        ...
        reply_text = model.generate_reply(system_prompt=system_prompt, history=history, user_message=message)
...
def task_feedback(*, level: Optional[str], message: str) -> Dict[str, Any]:
    model = _get_model()
    system_prompt = get_prompt(level, kind="feedback")
    raw = model.generate_feedback_raw(system_prompt=system_prompt, user_message=message)
    ...
    if parsed is None:
        parsed = fallback_language_feedback("Feedback temporarily unavailable (formatting error).")
